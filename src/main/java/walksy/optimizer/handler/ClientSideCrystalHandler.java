...
public class ClientSideCrystalHandler {
    ...
    public static void trySpawnClientCrystal(ClientWorld simulatedWorld, double posX, double posY, double posZ) {
        ...
        ClientCrystalEntity candidateCrystal = FACTORY.create(simulatedWorld, posX, posY, posZ);

        //Checks if the client crystal already exists
        if (!isAlreadyTracked(candidateCrystal)) {
            log("Attempting clientside crystal spawn at: " + candidateCrystal.getPos());
            performSpawnSimulation(candidateCrystal, simulatedWorld);
        }
    }
